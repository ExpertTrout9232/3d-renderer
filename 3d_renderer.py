...
for object in objects:
    ...
    object.append("faces_projection", [])

    for face in object["faces"]:
        v0, v1, v2 = [object["vertices"][i] for i in face]

        normal = (np.array(v1) - np.array(v0)) @ (np.array(v2) - np.array(v0))

        if np.dot(normal, np.array(v0)) < 0:
            object["faces_projection"].append({ "face": face, "normal": normal })
